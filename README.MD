...
```mermaid
flowchart TD;
    A[Your stored embeddings<br/>(clustered into nlist groups)];
    ...
    E --> F[Compute similarity (IP)<br/>inside those clusters only];
    F --> G[Rank top 5 nearest items<br/>by highest similarity];
    G --> H[Return results];
```
